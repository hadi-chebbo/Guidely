  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        eyebrow="People"
        title="Mentors"
        description="Review mentor accounts, search records, and manage blocked access."
      />
      ...
      <AdminToolbar>
        ...
        <div className="text-sm text-gray-500">
          {search.trim().length >= 2
            ...
            : `Showing ${data.length} of ${total} mentors`}
        </div>
      </AdminToolbar>
          ...
          <>
            <div className="hidden lg:block">
              <Table>
                ...
                <TBody>
                  {isEmpty ? (
                    <TableEmpty colSpan={5}>No mentors found.</TableEmpty>
                  ) : (
                    data.map((u) => (
          ...
          <div className="grid gap-3 p-4 lg:hidden">
            {isEmpty ? (
              <div className="rounded-lg border border-dashed border-gray-200 p-8 text-center">
                <h2 className="text-base font-bold text-gray-950">No mentors found</h2>
                <p className="mt-1 text-sm text-gray-500">
                  Try another search term or check back after mentor approval.
                </p>
              </div>
            ) : (
              data.map((u) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate font-semibold text-gray-950">{u.name}</p>
                      <p className="mt-1 truncate text-sm text-gray-500">@{u.username}</p>
                      <p className="mt-1 break-words text-sm text-gray-600">{u.email}</p>
                    </div>
                    <Badge variant={u.is_blocked ? "danger" : "success"}>
                      {u.is_blocked ? "Blocked" : "Active"}
                    </Badge>
                  ...
                  <div className="mt-4 flex justify-end gap-2">
                    <button
                      ...
                      className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-600 transition hover:bg-brand-50 hover:text-brand-700"
                      aria-label={`View ${u.name}`}
                      title={`View ${u.name}`}
                    >
                      <Eye className="h-4 w-4" />
                    ...
                    <button
                      ...
                      aria-label={u.is_blocked ? `Unblock ${u.name}` : `Block ${u.name}`}
                      title={u.is_blocked ? `Unblock ${u.name}` : `Block ${u.name}`}
                    >
                      {u.is_blocked ? (
                        <ShieldCheck className="h-4 w-4 text-green-600" />
                      ) : (
